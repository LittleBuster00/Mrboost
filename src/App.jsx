...
// ─── SUPABASE CONFIG ──────────────────────────────────────────────────────────
const SUPABASE_URL = "https://jcqtvdrkivwpaosuudam.supabase.co";
const SUPABASE_KEY = "sb_publishable_OTIuyj3Hm0t07RlikAATEw_aKRuwe_T";
...
async function sSave(k,v){
  try{
    const { error } = await supabase.from("kv_store").upsert({ key: k, value: v, updated_at: new Date().toISOString() });
...
export default function App() {
  ...
  useEffect(()=>{
    let mounted = true;
    (async()=>{
      const w=await sGet("pr2_w"); if(mounted && w) setW(w);
      const l=await sGet("pr2_l"); if(mounted && l) setL(l);
      const p=await sGet("pr2_p"); if(mounted && p) setP(p);
